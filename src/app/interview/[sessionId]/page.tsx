...
import { Send, Bot, User, CheckCircle, Video, VideoOff, Mic, Clock, ShieldOff, Home } from "lucide-react";
import { MAX_QUESTION_COUNT, CHAT_QUESTION_COUNT } from "@/lib/interview-config";
import { CODING_LANGUAGES, CODING_QUESTIONS, CodingLanguage } from "@/lib/coding-round";
...
export default function InterviewPage() {
  ...
  const [selectedCodingLanguage, setSelectedCodingLanguage] = useState<CodingLanguage>("python");
  const [activeQuestionIds, setActiveQuestionIds] = useState<string[]>(CODING_QUESTIONS.slice(0, 3).map((q) => q.id));
  ...
  const [questionCompletion, setQuestionCompletion] = useState<boolean[]>(
    CODING_QUESTIONS.map(() => false)
  );
  const [codingTimeLeft, setCodingTimeLeft] = useState(CODING_ROUND_SECONDS);
  const [codingCode, setCodingCode] = useState(
    CODING_QUESTIONS[0].starterByLanguage[selectedCodingLanguage]
  );
  const [runInput, setRunInput] = useState(CODING_QUESTIONS[0].tests[0]?.input ?? "");
  ...
  const activeQuestions = activeQuestionIds
    .map((id) => CODING_QUESTIONS.find((q) => q.id === id))
    .filter(Boolean) as typeof CODING_QUESTIONS;
  const currentCodingQuestion = activeQuestions[currentCodingQuestionIndex] ?? CODING_QUESTIONS[0];
      ...
      try {
        ...
        if (isSubmit) {
          ...
          if (currentCodingQuestionIndex < activeQuestions.length - 1) {
            ...
            setRunInput(nextQ.tests[0]?.input ?? "");
            setRunOutput("");
  ...
  const handleSendMessage = useCallback(async (customInput?: string, isInitial: boolean = false, hasVideo: boolean = false) => {
    ...
    if (!isInitial) {
      ...
      setMessages((prev) => [...prev, { id: msgId, role: "user", text: textToSend, hasVideo }]);
      setInput("");
    }
    setLoading(true);
  ...
  useEffect(() => {
    ...
    // If auto-submitted due to cheating, cancel the interview session and stop here.
    if (autoSubmitReason) {
    ...
    initInterview();
  }, [sessionId, handleSendMessage, codingRoundCompleted, autoSubmitReason]);
        ...
        {!codingRoundStarted && !showCodingCaution ? (
          <div className="h-full w-full flex items-center justify-center p-6">
            <div className="w-full max-w-3xl bg-white/5 border border-white/10 rounded-3xl p-8">
              <h1 className="text-2xl md:text-3xl font-bold">Coding Round - Full Stack Engineer</h1>
              ...
              <div className="mt-6 bg-slate-900/50 border border-white/10 rounded-2xl p-6">
                  ...
                  {CODING_LANGUAGES.map((lang) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setCodingCode(CODING_QUESTIONS[0].starterByLanguage[lang.id]);
                      }}
                ...
                <button
                  onClick={() => {
                    // Pick 3 random Medium questions each time starting the round
                    const mediumPool = CODING_QUESTIONS.filter((q) => q.difficulty === "Medium");
                    const picked: string[] = [];
                    const pool = [...mediumPool];
                    while (picked.length < 3 && pool.length) {
                      const idx = Math.floor(Math.random() * pool.length);
                      const [q] = pool.splice(idx, 1);
                      picked.push(q.id);
                    }
                    setActiveQuestionIds(picked.length === 3 ? picked : CODING_QUESTIONS.slice(0, 3).map((q) => q.id));
                    setCurrentCodingQuestionIndex(0);
                    setQuestionCompletion([false, false, false]);
                    const first = (picked.length === 3 ? picked : CODING_QUESTIONS.slice(0, 3).map((q) => q.id))[0];
                    const q0 = CODING_QUESTIONS.find((q) => q.id === first) ?? CODING_QUESTIONS[0];
                    setCodingCode(q0.starterByLanguage[selectedCodingLanguage]);
                    setRunInput(q0.tests[0]?.input ?? "");
  ...
  return (
    <div className="h-[100svh] w-full bg-slate-950 text-white flex flex-col overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-white leading-tight">AI Interview Session</h2>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400">
            {candidateResponsesCount}/{MAX_QUESTION_COUNT} responses
          </span>
          <button
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-200">AI Recruiter</span>
            </div>
            {(loading || isSpeaking) && (
      ...
      <div className="shrink-0 flex flex-col" style={{ maxHeight: "40%", borderTop: "1px solid rgba(255,255,255,0.06)" }}>

        {/* Messages scroll area */}
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0" style={{ maxHeight: "200px" }}>
          {messages.map((msg) => (
            <motion.div
              key={msg.id}
            ...
            >
              ...
              <div className={`max-w-[75%] rounded-xl px-3.5 py-2 text-[13px] leading-relaxed ${
                msg.role === "user"
        ...
        >
          ...
          >
            <input
              ...
              disabled={loading}
              placeholder={isVideoPhase ? "Type or record your response..." : "Type your answer..."}
            ...
            <button
              ...
              className="absolute right-1.5 p-1.5 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-700 text-white rounded-full transition-colors"
            >
